...
public class DaoArtista {
    ...
    public Boolean agregarArtista(EArtista artista){    
        try {
            ...
            procedimiento.setDate(3, convert(artista.getFechaNacimiento()));
    ...
    public Boolean editarArtista(EArtista artista){    
        try {
            System.out.println("entro");
            Class.forName("org.postgresql.Driver");
            Connection conectar = DriverManager.getConnection(db, usuarioP, contrasenaServer);
            CallableStatement procedimiento = conectar.prepareCall("{call public.f_editarartista(?,?,?,?,?)}");
            procedimiento.setInt(1, artista.getId());
            procedimiento.setString(2, artista.getNombre());
            procedimiento.setString(3, artista.getGenero());
            procedimiento.setDate(4, convert(artista.getFechaNacimiento()));
            procedimiento.setString(5, artista.getFoto());
            ResultSet rs = procedimiento.executeQuery();
            while(rs.next()){
                return rs.getBoolean(1);
            }
    ...
    public Boolean editarAlbum(EAlbum album){    
        try {
            System.out.println("entro");
            Class.forName("org.postgresql.Driver");
            Connection conectar = DriverManager.getConnection(db, usuarioP, contrasenaServer);
            CallableStatement procedimiento = conectar.prepareCall("{call public.f_editaralbum(?,?,?)}");
            procedimiento.setInt(1, album.getId());
            procedimiento.setString(2, album.getNombre());
            procedimiento.setInt(3, album.getValor());
            ResultSet rs = procedimiento.executeQuery();
            while(rs.next()){
                return rs.getBoolean(1);
            }
    ...
    public Boolean editarCancion(ECancion cancion){    
        try {
            System.out.println("entro");
            Class.forName("org.postgresql.Driver");
            Connection conectar = DriverManager.getConnection(db, usuarioP, contrasenaServer);
            CallableStatement procedimiento = conectar.prepareCall("{call public.f_editarcancion(?,?,?)}");
            procedimiento.setInt(1, cancion.getId());
            procedimiento.setString(2, cancion.getNombre());
            procedimiento.setInt(3, cancion.getValor());
            ResultSet rs = procedimiento.executeQuery();
            while(rs.next()){
                return rs.getBoolean(1);
